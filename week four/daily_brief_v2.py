...
def load_api_key():
    ...
    if not key:
        ...
        sys.exit(1)
    return key, Groq(api_key=os.getenv("GROQ_API_KEY"))
...
def rewrite_headlines(client, title): #function that uses groq to rewrite the headlines in a more fun way, maybe like a clickbait style? 
    ...
    try:
        ...
        temperature = 0.3,
            messages=[
                {"role": "system", "content": "You are a friendly summarizer. Rewrite this news headline in one sentence for a 10th grader who isn't into tech. Keep it under 20 words. Output only the rewritten sentence, nothing else."},
...
def render_panel(category, articles, client):
    ...
    for i, article in enumerate(articles, start=1):
        ...
        rewritten_title = rewrite_headlines(client, title)

        lines.append(f"[bold cyan]{i}. Original:[/bold cyan]")
        lines.append(title)

        lines.append(f"[bold green]AI Rewrite:[/bold green]")
        lines.append(rewritten_title)
...
def main():
    ...
    api_key, client = load_api_key()
    articles = fetch_headlines(api_key, category, DEFAULT_COUNTRY)
    if articles is None:
        sys.exit(1)
    render_panel(category, articles, client)
